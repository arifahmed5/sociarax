...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialMode = 'login' }) => {
  const { user, loginUser, loginWithGoogle, registerUser } = useAuth();
  ...
  // Auto-close if already logged in as customer
  useEffect(() => {
    if (user && isOpen) {
      onClose();
    }
  ...
  // If modal is closed or user is already logged in, do not render modal
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const res = await loginWithGoogle(idToken, result.user.email || undefined, result.user.displayName || undefined);
      if (res.success) {
        onClose();
      } else {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="mb-4 space-y-2.5">
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl bg-slate-950 hover:bg-slate-850 border border-slate-700/80 hover:border-slate-600 text-white font-medium text-xs sm:text-sm flex items-center justify-center gap-3 transition-all cursor-pointer shadow-md shadow-black/20 disabled:opacity-50"
          >
            ...
            <span>{isGoogleLoading ? 'Connecting to Google...' : (mode === 'login' ? 'Sign in with Google' : 'Sign up with Google')}</span>
          </button>
