  ...
  return (
    <header className="sticky top-0 z-40 bg-slate-950/95 backdrop-blur-md border-b border-slate-800">
      <div className="w-full max-w-7xl 2xl:max-w-[1700px] mx-auto px-2 sm:px-4 lg:px-6">
        <div className="flex items-center justify-between h-16 sm:h-18 gap-1.5 sm:gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3 shrink-0">
            ...
            >
              <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-tr ${theme.brandGradient} flex items-center justify-center shadow-md ${theme.brandGlow} group-hover:scale-105 transition-transform shrink-0`}>
                <Zap className="w-4 h-4 text-white" />
              </div>
              <span className={`font-extrabold text-sm sm:text-base text-white tracking-tight ${isOwnerOrAdmin ? 'hidden min-[420px]:inline' : ''}`}>
            ...
            {isOwnerOrAdmin && (
              <div className="flex items-center bg-slate-900/90 border border-slate-800 p-0.5 rounded-xl shrink-0 animate-fadeIn">
                ...
                >
                  <ShieldCheck className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-rose-400" />
                  <span>Owner</span>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {isAdminTab && admin ? (
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <button
                  ...
                  className="hidden sm:flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 hover:border-rose-500/50 rounded-xl text-xs font-semibold transition-all cursor-pointer shadow-xs shrink-0"
              ...
              <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                ...
                <button
                  ...
                  className="flex items-center gap-1 sm:gap-1.5 bg-slate-900 hover:bg-slate-850 border border-slate-800 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl transition-all cursor-pointer shrink-0"
                  title="View Wallet Balance"
                >
                  <Wallet className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400 shrink-0" />
                  <span className="font-bold text-[11px] sm:text-sm text-emerald-400 font-mono">
                ...
                {/* User Logout Button (Shows small arrow icon on mobile, full button on desktop) */}
                <button
                  onClick={logoutUser}
                  className="flex items-center gap-1.5 p-2 sm:px-3 sm:py-1.5 bg-slate-900 hover:bg-rose-500/10 text-slate-300 hover:text-rose-400 border border-slate-800 hover:border-rose-500/30 rounded-xl text-xs font-semibold transition-all cursor-pointer shadow-xs shrink-0"
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  ...
                  className="px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 text-white text-xs sm:text-sm font-semibold shadow-md shadow-indigo-600/30 transition-all cursor-pointer whitespace-nowrap shrink-0"
                >
            ...
            {/* Mobile Menu Button - 3 Lines (Always prominent, never clipped off-screen) */}
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="lg:hidden p-2 text-slate-200 bg-slate-900 hover:bg-slate-800 border border-slate-750 hover:border-slate-700 rounded-xl cursor-pointer shrink-0 transition-colors shadow-xs"
              aria-label="Toggle navigation menu"
              title="Toggle Menu"
            >
              {isMobileMenuOpen ? <X className="w-5 h-5 text-rose-400" /> : <Menu className="w-5 h-5 text-indigo-400" />}
            </button>
          </div>
        </div>
      </div>
